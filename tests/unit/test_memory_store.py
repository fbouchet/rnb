...
class TestUserMemoryOperations:
    ...
    def test_get_user_memories(self, memory_store):
        ...
        memory_store.add_user_memory("agent_001", "User prefers concise explanations")
        memory_store.add_user_memory("agent_001", "User has expertise in Python")
    ...
    def test_search_user_memories(self, memory_store):
        ...
        memory_store.add_user_memory(
            "agent_001", "User prefers Python for data science"
        )
        memory_store.add_user_memory("agent_001", "User likes detailed code examples")

        # Search for programming preferences
        results = memory_store.get_user_memories(
            "agent_001", query="programming language preferences", n_results=5
        )
...
class TestSessionMemoryOperations:
    ...
    def test_get_session_memories(self, memory_store):
        ...
        memories = memory_store.get_session_memories(
            "agent_001", session_id="conv_001", n_results=10
        )
...
class TestTaskMemoryOperations:
    ...
    def test_get_task_memories(self, memory_store):
        ...
        memory_store.add_task_memory(
            "agent_001", "Task: Write unit tests", metadata={"status": "completed"}
        )
        ...
        active_tasks = memory_store.get_task_memories(
            "agent_001", status="in_progress", n_results=10
        )
...
class TestMemorySearch:
    ...
    def test_search_all_memory_types(self, memory_store):
        ...
        memory_store.add_user_memory("agent_001", "User prefers Python programming")
        memory_store.add_session_memory("agent_001", "Discussed Python best practices")
        memory_store.add_task_memory("agent_001", "Task: Write Python script")

        # Search across all types
        results = memory_store.search_memories(
            agent_id="agent_001", query="Python programming", n_results=5
        )

"""
Tests for Phrase Parser with SO-CAL Negation Handling

Tests cover:
1. NegationInfo and SO-CAL shift semantics
2. ParsedPhrase intensity computation
3. PhraseParser with SpaCy integration
4. SO-CAL paper examples validation
5. Integration tests

Requires: pytest, spacy, en_core_web_sm
...
class TestNegationInfo:
    ...
    def test_clamping(self):
        ...
        # 0.2 - 1.5 = -1.3 → clamped to -1.0
        result = neg.apply_shift(0.2)
        assert result == -1.0
...
class TestSOCALPaperExamples:
    ...
    def test_not_sleazy(self):
        """'not sleazy' should be less negative."""
        from rnb.resources.phrase_parser import NegationInfo
...
# =============================================================================
# PhraseParser Tests with SpaCy
# =============================================================================


class TestPhraseParser:
    """Tests for PhraseParser with SpaCy."""
    ...
    def test_negated_modified_adjective(self, parser):
        """Parse 'not very lazy'."""
    ...
    def test_parse_list(self, parser):
        ...
        results = parser.parse_list(
            ["lazy", "very organized", "not creative", "not very ambitious"]
        )
        assert len(results) == 4
        assert results[0].adjective == "lazy"
        assert results[1].is_modified
        assert results[2].is_negated
        assert results[3].is_modified and results[3].is_negated
...
class TestEdgeCases:
    ...
    def test_multi_word_modifier(self, parser):
        """Multi-word modifier extraction."""
        result = parser.parse("a little bit nervous")
        assert result.adjective == "nervous"
        assert result.modifier == "a little bit"
        assert result.modifier_value == pytest.approx(-0.5)

    def test_contracted_negation(self, parser):
        """Contracted negation detection."""
        result = parser.parse("isn't happy")
        assert result.is_negated
        # SpaCy tokenizes "isn't" → "is" + "n't"
        assert result.negation.negation_type.value in ("standard", "contracted")

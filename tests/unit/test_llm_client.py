...
from rnb.llm.exceptions import LLMException, ModelNotAvailableError, RateLimitError
...
def test_unsupported_provider_raises_error():
    ...
    with pytest.raises(ModelNotAvailableError):
        LLMClient(provider="unsupported", model_name="test")  # Invalid provider
...
def test_query_with_system_prompt(mock_openai_client, mock_instructor):
    ...
    client.query("User prompt", system_prompt="You are a helpful assistant.")
...
def test_query_structured(mock_openai_client, mock_instructor):
    ...
    result = client.query_structured("Test prompt", response_model=SampleResponse)
...
def test_anthropic_query_format(mock_anthropic_client, mock_instructor):
    ...
    client.query("User prompt", system_prompt="System instruction")

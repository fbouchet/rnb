...
from rnb.resources import PersonalityResolver, SchemeRegistry
...
class TestPersonalityStateFactory:
    ...
    def test_from_adjectives_unknown(self, factory):
        ...
        state = factory.from_adjectives("test-agent", ["romantic", "xyzunknown"])
    ...
    def test_from_traits(self, factory):
        ...
        state = factory.from_traits(
            "test-agent", {Trait.OPENNESS: 0.8, Trait.CONSCIENTIOUSNESS: 0.6}
        )
        ...
        for _scheme_key, value in openness_schemes.items():
            assert value == 0.8
    ...
    def test_from_facets(self, factory):
        ...
        state = factory.from_facets(
            "test-agent", {"Openness_Fantasy": 0.9, "Conscientiousness_Order": 0.7}
        )

        # All schemes under Fantasy should be 0.9
        fantasy_schemes = state.get_schemes_for_facet("Openness", "Fantasy")
        for _scheme_key, value in fantasy_schemes.items():
            assert value == 0.9
    ...
    def test_from_mixed(self, factory):
        ...
        spiritualist = state.get_scheme(
            "Openness_Fantasy_SPIRITUALISTNESS", default=None
        )
...
class TestArchetypes:
    ...
    def test_archetype_structure(self, factory):
        ...
        for _trait_name, value in archetype.traits.items():
...
class TestIntegration:
    ...
    def test_trait_propagation(self, factory):
        ...
        for _scheme_key, value in state.get_schemes_for_trait(Trait.OPENNESS).items():
            assert value == 0.7

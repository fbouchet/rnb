...
def llm_client():
    ...
    return LLMClient(
        provider=ModelProvider.LOCAL, model_name="llama3.2:3b", timeout=120.0
    )
...
class TestUserMemoryIntegration:
    ...
    def test_user_preference_affects_response(
        ...
        response_without_memory = llm_client.query(
            base_prompt, temperature=0.7, max_tokens=200
        )
        ...
        memory_context = "\n".join(
            [f"User preference: {mem['content']}" for mem in user_prefs]
        )
...
Respond according to the user's stated preferences."""
        ...
        response_with_memory = llm_client.query(
            behavioral_prompt, temperature=0.7, max_tokens=200
        )
    ...
    def test_user_expertise_affects_explanation_level(
        self, agent_manager, memory_store, operator_registry, llm_client
    ):
        """
...
Adjust explanation level accordingly."""
            ...
            response = llm_client.query(
                augmented_query, temperature=0.7, max_tokens=250
            )
            ...
            has_beginner_language = any(
                word in response_lower for word in beginner_indicators
            )
            assert (
                has_beginner_language
            ), "Expected beginner-friendly explanation based on user expertise"


class TestSessionMemoryIntegration:
    """Test M.S (Session Model) integration with LLM"""

    def test_conversation_continuity(self, agent_manager, memory_store, llm_client):
        """
        ...
        memory_store.add_session_memory(
            ...
            metadata={"session_id": session_id, "turn": 1, "topic": "recursion"},
        )
        ...
        history = memory_store.get_session_memories(
            agent_id="tutor_003", session_id=session_id, n_results=5
        )
...
class TestTaskMemoryIntegration:
    ...
    def test_task_tracking_across_turns(self, agent_manager, memory_store, llm_client):
        """
        ...
        # Turn 2: Show pseudocode
        query2 = "Show me the pseudocode for binary search"
        response2 = llm_client.query(query2, temperature=0.7, max_tokens=200)

        # Update task: pseudocode shown
        memory_store.add_task_memory(
            agent_id="tutor_004",
            content="Task progress: Showed binary search pseudocode",
            metadata={
                "task_id": task_id,
                "status": "in_progress",
                "steps_completed": "explain_concept, show_pseudocode",
                "steps_remaining": "write_code",
            },
        )
        ...
        response3 = llm_client.query(query3, temperature=0.7, max_tokens=300)
        ...
        task_memories = memory_store.get_task_memories(
            agent_id="tutor_004", status="in_progress", n_results=5
        )
        ...
        assert (
            "explain_concept" in steps_completed
...
{personality_prompt}"""
        ...
        print("Personality: High conscientiousness (structured, detailed)")
        print("Memory: User prefers Python examples")
        print("\nResponse:")
        print(response)
        ...
        has_structure = any(
            word in response_lower for word in ["first", "then", "step", "finally"]
        )

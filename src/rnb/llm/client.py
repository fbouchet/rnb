...
class LLMClient:
    ...
    def __init__(
        ...
        try:
            if provider == ModelProvider.OPENAI:
                ...
                # Keep both raw and instructor-wrapped clients
                self.raw_client = Anthropic(api_key=api_key, timeout=timeout)
            ...
            if isinstance(e, ModelNotAvailableError):
                raise
            raise ModelNotAvailableError(
                provider=provider, model=model_name, details=str(e)
            ) from e
    ...
    def query(
        ...
        try:
            ...
            raise RateLimitError(provider=self.provider.value) from e
        except (APIError, AnthropicAPIError) as e:
            raise LLMException(f"API error: {str(e)}") from e

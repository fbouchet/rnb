...
class AdjectiveResolver:
    ...
    def from_yaml(
        ...
        logger.info(f"Loaded {resolver.statistics['unique_adjectives']} unique adjectives")
    ...
    def search(
        ...
        matches = [
            word for word in self._adjective_index.keys()
    ...
    def get_adjectives_for_facet(
        ...
        for entries in self._adjective_index.values():
            for entry in entries:
                if (entry.position.trait.lower() == trait.lower() and
                    entry.position.facet.lower() == facet.lower()):
                    if target_pole is None or entry.position.pole == target_pole:
                        results.append(entry)
        
        return results
    
    def get_adjectives_by_weight(
        self, 
        min_weight: int = 1
    ) -> list[AdjectiveEntry]:
        """
    ...
    def statistics(self) -> dict:
        ...
        return {
            ...
            "avg_mappings_per_adjective": (
                ...
                if self._adjective_index else 0
            ),
            "weight_distribution": {
                w: weights.count(w) for w in range(1, 10)
            },
            "max_weight_adjectives": [
                word for word, entries in self._adjective_index.items()

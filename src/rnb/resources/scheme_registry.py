...
class SchemeRegistry:
    ...
    def get_scheme_by_key(self, scheme_key: str) -> Optional[SchemeInfo]:
        """
    ...
    def get_position_for_gloss(
        self, 
        gloss_id: str
    ) -> Optional[TaxonomyPosition]:
        """
    ...
    def get_glosses_for_scheme(
        ...
        facet: str, 
        scheme_name: str,
        pole: Optional[Literal["pos", "neg"]] = None
    ) -> list[GlossEntry]:
        """
    ...
    def get_schemes_for_facet(
        ...
        return [
            scheme for key, scheme in self._schemes.items()
    ...
    def get_schemes_for_trait(self, trait: str) -> list[SchemeInfo]:
        ...
        return [
            scheme for scheme in self._schemes.values()
    ...
    def get_schemes_by_operator_hint(
        self, 
        operator_hint: str
    ) -> list[SchemeInfo]:
        """
    ...
    def get_facets_for_trait(self, trait: str) -> list[str]:
        ...
        trait_lower = trait.lower()
        facets = {
            scheme.facet for scheme in self._schemes.values()
            if scheme.trait.lower() == trait_lower
        }
    ...
    def statistics(self) -> dict:
        ...
        wordnet_glosses = sum(
            1 for gid in self._gloss_to_position.keys()
            if not gid.startswith("Q")
        )
        ...
        return {
            ...
            "unique_operator_hints": len(set(
                s.operator_hint for s in self._schemes.values()
            )),

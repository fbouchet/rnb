...
class PersonalitySpecification:
    ...
    def affected_schemes(self) -> set[str]:
        ...
        """
        return {r.position.scheme_key for r in self.resolved if r.position.scheme}
    ...
    def get_resolved_for_facet(self, trait: str, facet: str) -> list[ResolvedAdjective]:
        """
        ...
        return [r for r in self.resolved if r.position.facet_key == facet_key]
    ...
    def get_resolved_for_trait(self, trait: str) -> list[ResolvedAdjective]:
        ...
        """
        return [r for r in self.resolved if r.position.trait.lower() == trait.lower()]
...
class PersonalityResolver:
    ...
    def from_yaml(
        ...
        adj_resolver = AdjectiveResolver.from_yaml(
            neopiradj_path, warn_unresolved=warn_unresolved
        )
    ...
    def resolve(self, adjectives: list[str]) -> PersonalitySpecification:
        """
            ...
            for mapping in resolution.mappings:
                ...
                if full_position is None:
                    ...
                    scheme = self.schemes.get_scheme(
                        full_position.trait, full_position.facet, full_position.scheme
                    )

                    all_glosses = (
                        self.schemes.get_glosses_for_scheme(
                            full_position.trait,
                            full_position.facet,

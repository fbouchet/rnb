...
from .gloss_engine import ContextStyle, GlossInfluenceEngine
...
class GlossBasedInfluence(InfluenceOperator):
    ...
    def __init__(
        ...
        if name:
            ...
            trait_name = (
                filter_trait.value if isinstance(filter_trait, Trait) else filter_trait
            )
    ...
    def applies(self, context: InfluenceContext) -> bool:
        ...
        if self.filter_trait and active_schemes:
            trait_name = (
                self.filter_trait.value
                if isinstance(self.filter_trait, Trait)
                else self.filter_trait
            )
            active_schemes = [
                (key, val, pole)
                for key, val, pole in active_schemes
    ...
    def get_active_glosses_preview(self, state: PersonalityState) -> list[str]:
        """
        ...
        glosses = self.engine.get_active_glosses(
            state, threshold=self.threshold, filter_trait=self.filter_trait
        )

...
from .taxonomy import Trait, FACETS_BY_TRAIT
...
class PersonalityState:
    ...
    def _validate_ranges(self) -> None:
        ...
        for key, value in self.schemes.items():
            if not -1.0 <= value <= 1.0:
                raise ValueError(
                    f"Scheme {key} value must be in [-1, 1], got {value}"
                )
    ...
    def get_schemes_for_facet(
        ...
        return {
            k: v for k, v in self.schemes.items()
    ...
    def clone(self, new_agent_id: Optional[str] = None) -> "PersonalityState":
        """

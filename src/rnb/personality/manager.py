...
class AgentManager:
    ...
    def create_agent(
        self,
        agent_id: str,
        traits: Optional[dict[Trait, float]] = None,
        moods: Optional[dict[MoodDimension, float]] = None,
        affects: Optional[dict[AffectDimension, float]] = None
    ) -> PersonalityState:
        """
    ...
    def get_agent_summary(self, agent_id: str) -> dict:
        ...
        dominant_traits = [
            trait.value for trait, value in state.traits.items()
            if abs(value) > 0.5
        ]
    ...
    def apply_update_rule(
        self,
        agent_id: str,
        rule: Callable[[PersonalityState], PersonalityState]
    ) -> None:
        """
    ...
    def reset_agent_moods(self, agent_id: str) -> None:
        ...
        Raises:
            AgentNotFoundError: If agent doesn't exist
        """
        self.store.set_moods(
            agent_id,
            {mood: 0.0 for mood in MoodDimension}
        )
    ...
    def reset_agent_affects(self, agent_id: str) -> None:
        ...
        Raises:
            AgentNotFoundError: If agent doesn't exist
        """
        self.store.set_affects(
            agent_id,
            {affect: 0.0 for affect in AffectDimension}
        )
